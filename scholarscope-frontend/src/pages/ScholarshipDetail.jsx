...
import { useParams, useNavigate } from "react-router-dom";
import api from '../api'
import { Bookmark, BookmarkCheck, Heart, HeartHandshake, Share2, ArrowLeft, ChevronDown, ExternalLink, Check } from "lucide-react";
...
export default function ScholarshipDetail() {
    ...
    async function handleBookmark() {
        if (!scholarship) return;
        const prev = scholarship.is_bookmarked;
        setScholarship(s => ({ ...s, is_bookmarked: !prev }));
        try {
            await api.post(prev
    ...
    async function handleSave() {
        if (!scholarship) return;
        const prev = scholarship.is_saved;
        setScholarship(s => ({ ...s, is_saved: !prev }));
        try {
            await api.post(prev
                ? `scholarships/${id}/unsave_scholarship/`
    ...
    async function handleApply() {
        try {
            const res = await api.post(`scholarships/${id}/apply/`);
            const link = res.data.scholarship_link || scholarship.link;
            if (link) window.open(link, '_blank');
            setScholarship(s => ({ ...s, is_saved: true }));
        } catch (err) {
            handleAuthError(err);
            if (scholarship.link) window.open(scholarship.link, '_blank');
        }
    }
    ...
    async function handleShare() {
        ...
        try {
            if (navigator.share) {
                await navigator.share({ title: scholarship.title, url });
            } else {
        ...
        <div className="bg-background-light dark:bg-background-dark font-display text-slate-800 dark:text-slate-200 min-h-screen">
            ...
            <main className="max-w-5xl mx-auto px-6 py-10">
                ...
                <div className="mb-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {scholarship.tags?.map((tag, i) => (
                            <span key={i} className="px-3 py-1 rounded-lg bg-primary/15 text-primary text-sm font-medium">
                                {tag.name || tag}
                            </span>
                        ))}
                    </div>
                ...
                {/* ── Action bar ─────────────────────────────────────────────────────
                    Every button is labelled. An icon alone fails on mobile (no hover
                    tooltips) and is ambiguous ("is this a heart or a save?").
                    Active states use solid colour fill, not just a border change.
                ──────────────────────────────────────────────────────────────────── */}
                <div className="flex flex-wrap items-stretch gap-3 mb-10">
                    ...
                    <button
                        onClick={handleApply}
                        className="flex h-12 flex-1 min-w-[140px] items-center justify-center gap-2 rounded-lg bg-primary text-white font-semibold shadow-sm hover:bg-primary/90 transition"
                    >
                        <ExternalLink className="w-4 h-4" />
                        Apply Now
                    </button>

                    {/* Bookmark — labelled, solid fill when active */}
                    <button
                        onClick={handleBookmark}
                        className={`flex h-12 items-center justify-center gap-2 px-4 rounded-lg border font-semibold text-sm transition-all duration-150 ${
                            scholarship.is_bookmarked
                        ...
                        title={scholarship.is_bookmarked ? "Remove bookmark" : "Bookmark this scholarship"}
                        aria-pressed={scholarship.is_bookmarked}
                    >
                        {scholarship.is_bookmarked
                            ? <BookmarkCheck className="w-4 h-4" />
                            : <Bookmark className="w-4 h-4" />
                        }
                        <span>{scholarship.is_bookmarked ? "Bookmarked" : "Bookmark"}</span>
                    </button>

                    {/* Save / Shortlist — distinct from Bookmark */}
                    <button
                        onClick={handleSave}
                        className={`flex h-12 items-center justify-center gap-2 px-4 rounded-lg border font-semibold text-sm transition-all duration-150 ${
                            scholarship.is_saved
                        ...
                        title={scholarship.is_saved ? "Remove from shortlist" : "Add to shortlist"}
                        aria-pressed={scholarship.is_saved}
                    >
                        {scholarship.is_saved
                            ? <HeartHandshake className="w-4 h-4" />
                            : <Heart className="w-4 h-4" />
                        }
                        <span>{scholarship.is_saved ? "Shortlisted" : "Shortlist"}</span>
                    </button>

                    {/* Share */}
                    <button
                        onClick={handleShare}
                        className="flex h-12 items-center justify-center gap-2 px-4 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 font-semibold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition"
                        title="Share"
                    >
